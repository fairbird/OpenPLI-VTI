...
def InitParentalControl():
	...
	config.ParentalControl.storemoviepin = ConfigSelection(default = "5", choices = [("5", _("5 minutes")), ("30", _("30 minutes")), ("60", _("60 minutes")), ("standby", _("until standby/restart"))])
	config.ParentalControl.storedeletepin = ConfigSelection(default = "never", choices = [("never", _("never")),("5", _("5 minutes")), ("30", _("30 minutes")), ("60", _("60 minutes")), ("standby", _("until standby/restart"))])
	...
	config.ParentalControl.moviepinactive = ConfigYesNo(default = False)
	config.ParentalControl.moviepin = ConfigPIN(default = -1)
	config.ParentalControl.deletepinactive = ConfigYesNo(default = False)
	config.ParentalControl.deletepin = ConfigPIN(default = -1)
	...
	config.ParentalControl.retries.moviepin = ConfigSubsection()
	config.ParentalControl.retries.moviepin.tries = ConfigInteger(default = 3)
	config.ParentalControl.retries.moviepin.time = ConfigInteger(default = 3)
	config.ParentalControl.retries.deletepin = ConfigSubsection()
	config.ParentalControl.retries.deletepin.tries = ConfigInteger(default = 3)
	config.ParentalControl.retries.deletepin.time = ConfigInteger(default = 3)
#	config.ParentalControl.configured = configElement("config.ParentalControl.configured", configSelection, 1, (("yes", _("yes")), ("no", _("no"))))
	...
	config.ParentalControl.config_sections.timer_list = ConfigYesNo(default = False)
	...
	config.ParentalControl.config_sections.vti_panel = ConfigYesNo(default = False)
	config.ParentalControl.config_sections.vti_menu = ConfigYesNo(default = False)
...
class ParentalControlFolder:
	def __init__(self):
		self.configInitialized = False
		self.sessionPinTimer = eTimer()
		self.sessionPinTimer.callback.append(self.resetSessionPin)
		self.sessionDeletePinTimer = eTimer()
		self.sessionDeletePinTimer.callback.append(self.resetSessionDeletePin)

	def getConfigValues(self):
		self.checkDeletePinInterval = False
		self.checkSessionDeletePin = False
		self.sessionDeletePinCached = False
		self.deletePinIntervalSeconds = 0
		...
		self.storeMoviePin = config.ParentalControl.storemoviepin.value
		
		if self.storeMoviePin == "standby":
			self.checkSessionPin = True
		else:
			self.checkPinInterval = True
			iMinutes = float(self.storeMoviePin)
			iSeconds = iMinutes*60
			self.pinIntervalSeconds = int(iSeconds)
		if not self.configInitialized:
			config.ParentalControl.storemoviepin.addNotifier(self.configChangeCallbackMoviePin, initial_call = False)
		
		self.storeDeletePin = config.ParentalControl.storedeletepin.value
		if self.storeDeletePin == "never":
			pass
		elif self.storeDeletePin == "standby":
			self.checkSessionDeletePin = True
		else:
			self.checkDeletePinInterval = True
			iMinutes = float(self.storeDeletePin)
			iSeconds = iMinutes*60
			self.deletePinIntervalSeconds = int(iSeconds)
		if not self.configInitialized:
			config.ParentalControl.storedeletepin.addNotifier(self.configChangeCallbackDeletePin, initial_call = False)
		self.configInitialized = True
		
		config.misc.standbyCounter.addNotifier(self.standbyCounterCallback, initial_call = False)

	def configChangeCallbackMoviePin(self, configElement):
		self.getConfigValues()
		self.resetSessionPin()

	def configChangeCallbackDeletePin(self, configElement):
		self.getConfigValues()
		self.resetSessionDeletePin()
	...
	def resetSessionDeletePin(self):
		self.sessionDeletePinCached = False
		self.sessionDeletePinTimer.stop()

	def getCurrentTimeStamp(self):
		return time.time()

	def getPinList(self):
		return [ config.ParentalControl.moviepin.value ]

	def getDeletePinList(self):
		return [ config.ParentalControl.deletepin.value ]

	def moviePinEntered(self, result):
		if result is not None and result:
			if self.checkSessionPin == True:
				self.sessionPinCached = True
			if self.checkPinInterval == True:
	...
	def deletePinEntered(self, result):
		if result is not None and result:
			if self.checkSessionDeletePin == True:
				self.sessionDeletePinCached = True
			if self.checkDeletePinInterval == True:
				self.sessionDeletePinCached = True
				self.sessionDeletePinTimer.start(self.deletePinIntervalSeconds*1000,1)

parentalControlFolder = ParentalControlFolder()
