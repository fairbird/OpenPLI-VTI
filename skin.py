...
from enigma import eSize, ePoint, gFont, eWindow, eLabel, ePixmap, eWindowStyleManager, addFont, gRGB, eWindowStyleSkinned
...
from enigma import runMainloop, eDVBDB, eTimer, quitMainloop, getDesktop, ePythonConfigQuery, eAVSwitch, eServiceEvent
pngcache = []
...
def applySingleAttribute(guiObject, desktop, attrib, value, scale = ((1, 1), (1, 1))):
    try:
        if attrib == 'position':
            ...
            try:
                guiObject.setOrientation(*{'orVertical': (guiObject.orVertical, False),
                 ...
                 'orRightToLeft': (guiObject.orHorizontal, True)}[value])
            except KeyError:
                print 'oprientation must be either orVertical or orHorizontal!'

        elif attrib == 'valign':
            try:
                guiObject.setVAlign({'top': guiObject.alignTop,
                 'center': guiObject.alignCenter,
                 'bottom': guiObject.alignBottom}[value])
            except KeyError:
                print 'valign must be either top, center or bottom!'

        elif attrib == 'halign':
            try:
                guiObject.setHAlign({'left': guiObject.alignLeft,
                 'center': guiObject.alignCenter,
                 'right': guiObject.alignRight,
                 'block': guiObject.alignBlock}[value])
            except KeyError:
            ...
            guiObject.setScrollbarMode({'showOnDemand': guiObject.showOnDemand,
             'showAlways': guiObject.showAlways,
             'showNever': guiObject.showNever}[value])
            ...
            guiObject.setPointer({'pointer': 0,
             'seek_pointer': 1}[attrib], ptr, pos)
